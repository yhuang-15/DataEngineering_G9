...
def create_table(table_name):
    ...
    return json.dumps({'message': f'{table_name} table was created'}, sort_keys=False, indent=4), 200


@app.route('/training-db/<table_name>', methods=['DELETE'])
def delete_table(table_name):
    db_util.drop_tb(table_name)
    return json.dumps({'message': f'{table_name} table was dropped'}, sort_keys=False, indent=4), 200
...
def update_data(table_name):
    ...
    return json.dumps({'message': f'training data from {table_name} were updated'}, sort_keys=False, indent=4), 200
